...
func (tm *TaskManager) UpdateTask(id int, title, description string, done bool) error {
	...
	// Update the task while preserving ID and CreatedAt
	task.Title = title
...
func (tm *TaskManager) ListTasks(filterDone *bool) []Task {
	...
	for _, task := range tm.tasks {
		// If no filter is provided, include all tasks
		if filterDone == nil {
			result = append(result, task)
		} else if task.Done == *filterDone {
			// Only include tasks that match the filter criteria
			result = append(result, task)
		}
	}
