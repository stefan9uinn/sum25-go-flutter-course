        ...
        return (
            <div>
                ...
                <div className="code-output">
                    {Object.keys(this.props.response).length === 0 || this.props.response.message === "Please try once again, there is an error in your code" ? Object.keys(this.props.db_state).length === 0 ? (<Typography.Text className='code-initial-text'>Current DB state will appear here</Typography.Text>) : (this.props.db_state.state.map((item, index) => {
                        return (
                            <div className="code-output-item">
                                ...
                                {index !== this.props.db_state.state.length - 1 && <Divider className="my-divider" />}
                            </div>
                        );})) : (this.props.response.db_state.map((item, index) => {
                        return (
                            <div className="code-output-item">
                                <Typography.Text className='code-text'>ID: <Typography.Text className='code-text' style={{ color: '#fff' }}>{item.id}</Typography.Text></Typography.Text> <br />
                                <Typography.Text className='code-text'>Title: <Typography.Text className='code-text' style={{ color: '#fff' }}>{item.document}</Typography.Text></Typography.Text><br />
